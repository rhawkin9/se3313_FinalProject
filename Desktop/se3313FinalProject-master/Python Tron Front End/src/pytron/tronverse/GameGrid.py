...
class GameGrid(object):
    ...
    def __init__(self, Width, Height):
        ...
        # Initialize pyGame.
        pygame.init()
        pygame.display.set_caption("SE3313 Final Project")
    ...
    def _DrawLightRibbon(self, Vertices, Color = RED):
        '''
    ...
    def _HandleEvents(self, Events):
        ...
        for e in Events:
            if e.type == QUIT:
                ...
                if e.key == K_a:
                    self._Programs[0].LightCycle.ChangeDirection(LEFT)
                elif e.key == K_s:
                    self._Programs[0].LightCycle.ChangeDirection(DOWN)
                elif e.key == K_d:
                    self._Programs[0].LightCycle.ChangeDirection(RIGHT)
                elif e.key == K_w:
                    self._Programs[0].LightCycle.ChangeDirection(UP)
                
                # Program two navigation.
                elif e.key == K_j:
                    self._Programs[1].LightCycle.ChangeDirection(LEFT)
                elif e.key == K_k:
                    self._Programs[1].LightCycle.ChangeDirection(DOWN)
                elif e.key == K_l:
                    self._Programs[1].LightCycle.ChangeDirection(RIGHT)
                elif e.key == K_i:
                    self._Programs[1].LightCycle.ChangeDirection(UP)
